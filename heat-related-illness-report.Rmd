...
#hri v2 syndrome looks slightly more sensitive - follow up to validate
heat_ed_api <- "https://essence.syndromicsurveillance.org/nssp_essence/api/timeSeries?endDate=15Jun2022&geography=il_cook&percentParam=ccddCategory&datasource=va_er&startDate=17Mar2022&medicalGroupingSystem=essencesyndromes&userId=660&site=890&aqtTarget=TimeSeries&ccddCategory=heat%20related%20illness%20v2&geographySystem=region&detector=probrepswitch&timeResolution=daily&hasBeenE=1"

ord_temp_api <- paste0("https://essence.syndromicsurveillance.org/nssp_essence/api/timeSeries?percentParam=noPercent&endDate=",
               essence_api_date(Sys.Date()),"&userId=2541&weatherFactor=maxtemp&datasource=va_weather_aggr&stationAggregateFunc=avg&timeResolution=daily&aqtTarget=TimeSeries&detector=probrepswitch&timeAggregateFunc=avg&startDate=",
               essence_api_date(Sys.Date() - 90),"&stationID=lot-ord") 

heat_details_api <- paste0("https://essence.syndromicsurveillance.org/nssp_essence/api/dataDetails/csv?geography=il_cook&percentParam=ccddCategory&datasource=va_er&medicalGroupingSystem=essencesyndromes&userId=660&site=890&aqtTarget=TimeSeries&ccddCategory=heat%20related%20illness%20v2&geographySystem=region&detector=probrepswitch&timeResolution=daily&hasBeenE=1&startDate=", essence_api_date(Sys.Date() - 90), "&endDate=", essence_api_date(Sys.Date()))

heat_percent <- nssp_get_time_series(heat_ed_api, startDate = essence_api_date(Sys.Date() - 90), endDate = essence_api_date(Sys.Date()))
...
plot_ly(data = ed_plot_data, x = ~date) %>%
  add_lines(y = ~count, yaxis = "y1", name = "% ED Vists for HRI") %>%
...
heat_age_api <- paste0("https://essence.syndromicsurveillance.org/nssp_essence/api/tableBuilder/?endDate=", 
                       essence_api_date(Sys.Date()), "&geography=il_cook&percentParam=ccddCategory&datasource=va_er&startDate=",
                       essence_api_date(Sys.Date() - 60),
                       "&medicalGroupingSystem=essencesyndromes&userId=660&site=890&aqtTarget=TableBuilder&ccddCategory=heat%20related%20illness%20v2&geographySystem=region&detector=probrepswitch&timeResolution=daily&hasBeenE=1&rowFields=age&columnField=timeResolution")
...
heat_race_api <- paste0("https://essence.syndromicsurveillance.org/nssp_essence/api/tableBuilder/?endDate=", 
                       essence_api_date(Sys.Date()), "&geography=il_cook&percentParam=ccddCategory&datasource=va_er&startDate=",
                       essence_api_date(Sys.Date() - 60),
                       "&medicalGroupingSystem=essencesyndromes&userId=660&site=890&aqtTarget=TableBuilder&ccddCategory=heat%20related%20illness%20v2&geographySystem=region&detector=probrepswitch&timeResolution=daily&hasBeenE=1&rowFields=cRaceEthBroad&columnField=timeResolution")
...
heat_zip_api <- paste0("https://essence.syndromicsurveillance.org/nssp_essence/api/tableBuilder?endDate=",
essence_api_date(Sys.Date()), "&geography=60004,60005,60007,60008,60010,60016,60018,60022,60025,60026,60029,60043,60053,60056,60062,60067,60068,60070,60074,60090,60091,60093,60107,60120,60169,60173,60192,60193,60194,60195,60712,60714,60006,60009,60017,60019,60038,60055,60065,60078,60082,60094,60095,60159,60168,60179,60196,60011,60409,60411,60419,60422,60425,60426,60428,60429,60430,60438,60443,60445,60452,60461,60469,60471,60472,60473,60475,60476,60477,60478,60487,60827,60412,60406,60415,60439,60453,60455,60456,60457,60458,60462,60463,60464,60465,60467,60480,60482,60501,60803,60805,60454,60499,60104,60130,60131,60141,60153,60154,60155,60160,60162,60163,60164,60165,60171,60176,60305,60402,60513,60525,60526,60534,60546,60558,60706,60707,60804,60161&percentParam=ccddCategory&datasource=va_er&startDate=",
essence_api_date(Sys.Date() - 60), "&medicalGroupingSystem=essencesyndromes&userId=660&site=890&aqtTarget=TableBuilder&ccddCategory=heat%20related%20illness%20v2&geographySystem=zipcode&detector=probrepswitch&timeResolution=daily&hasBeenE=1&rowFields=timeResolution&columnField=geographyzipcode")
